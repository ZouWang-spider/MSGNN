...
# 创建一个与模型结构匹配的实例
loaded_syn_gcn = GGNN_FCModel(input_size, hidden_size, num_layers, num_steps, num_etypes, num_classes)
# 加载模型参数
loaded_syn_gcn.load_state_dict(torch.load('word_ggnn_model.pth'))
# 将模型设置为评估模式（如果需要进行推断）
loaded_syn_gcn.eval()
...
# 存储标签和预测结果
true_labels = []  # 存储真实标签
predicted_labels = []  # 存储模型预测的标签

# Word_embedding_feature =  []
for i, (text, label) in enumerate(zip(Word_feature, train_label)):
        ...
        g = dgl.graph(graph)  # 句子的图结构
         ...
         #使用模型进行预测
        with torch.no_grad():
            predictions = loaded_syn_gcn(g, Word_embedding_feature)
        print(predictions)
        _, predicted = torch.max(predictions, 1)  # 获取最大值的索引为计算的类别

        # 存储真实标签和模型预测的标签
        true_labels.append(label)
        predicted_labels.append(predicted.item())  # 假设predicted是一个张量，使用item()方法获取标量值
