...
# 定义优化器和损失函数
dep_ggnn = GGNN_FCModel(input_size, hidden_size, num_layers, num_steps, num_etypes,num_classes)  # 替换成你的模型
optimizer = optim.Adam(dep_ggnn.parameters(), lr=learning_rate)
...
for epoch in range(num_epochs):
    ...
    for i, (text, label) in enumerate(zip(Word_feature[start_idx:end_idx],train_label[start_idx:end_idx])):
        ...
        rels = dataset.rels[0]  # 边的信息
        # print(rels)

        # 获取依存特征
        marked_text = ["[CLS]"] + rels + ["[SEP]"]
        ...
        # 提取单词（去掉特殊标记的部分）
        dep_list = [item for item in marked_text if item not in ['[CLS]', '[SEP]']]
        # 使用切片操作去除第一个和最后一个元素
        ...
        # Forward pass
        output = dep_ggnn(g, dep_embedding_feature)  # GGNN模型输入结果为图、节点特征  output.unsqueeze(0)
    ...
    # 打开文本文件以写入模式
    with open('dep_output.txt', 'a') as file:
