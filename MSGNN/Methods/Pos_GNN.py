...
# 定义优化器和损失函数
pos_ggnn = GGNN_FCModel(input_size, hidden_size, num_layers, num_steps, num_etypes,num_classes)  # 替换成你的模型
optimizer = optim.Adam(pos_ggnn.parameters(), lr=learning_rate)
...
for epoch in range(num_epochs):
    ...
    for i, (text, label) in enumerate(zip(POS_feature[start_idx:end_idx],train_label[start_idx:end_idx])):
        ...
        # 提取单词（去掉特殊标记的部分）
        pos_list = [item for item in marked_text if item not in ['[CLS]', '[SEP]']]
        # 使用切片操作去除第一个和最后一个元素
        ...
        # Forward pass
        output = pos_ggnn(g, pos_embedding_feature)  # GGNN模型输入结果为图、节点特征  output.unsqueeze(0)
    ...
    # 打开文本文件以写入模式
    with open('pos_output.txt', 'a') as file:
